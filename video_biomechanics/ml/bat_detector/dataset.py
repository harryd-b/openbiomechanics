"""Dataset for bat endpoint detection training."""
...
class SyntheticBatDataset(Dataset):
    """Dataset for synthetic bat images with endpoint annotations."""
    ...
    def __init__(
        ...
        print(f"Loaded {len(self.annotations)} synthetic bat annotations")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Prepare keypoints
        keypoints = [(float(x), float(y)) for x, y in ann['endpoints'][:2]]
        ...
        for i, (x, y) in enumerate(new_keypoints[:2]):
...
class BatDataset(Dataset):
    """Dataset for bat endpoint detection.

    Loads video frames and their endpoint annotations.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            visible_only: Only include frames where bat is visible
        """
        ...
        # Load annotations
        self.store = BatAnnotationStore.load(annotations_file)
        if visible_only:
            self.annotations = self.store.visible_annotations()
        else:
        ...
        print(f"Loaded {len(self.annotations)} annotations (visible_only={visible_only})")
    ...
    def _create_transform(self, augment: bool) -> A.Compose:
        ...
        if augment:
            return A.Compose([
                ...
                A.OneOf([
                    A.GaussianBlur(blur_limit=5),
                    A.MotionBlur(blur_limit=7),  # Motion blur common in fast swings
                ], p=0.4),
                A.OneOf([
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            ...
            keypoints: Tensor of shape (4,) with normalized coordinates [knob_x, knob_y, tip_x, tip_y]
        """
        ann = self.annotations[idx]
...
def create_dataloaders(
    ...
    video_train_dataset = BatDataset(
    ...
    if synthetic_annotations_file and synthetic_images_dir:
        if synthetic_annotations_file.exists() and synthetic_images_dir.exists():
            synthetic_dataset = SyntheticBatDataset(
...
if __name__ == "__main__":
    ...
    annotations_file = script_dir / "bat_annotations.json"
    ...
    dataset = BatDataset(annotations_file, data_dir.resolve(), augment=True)
    ...
    print(f"Keypoints (normalized): {keypoints}")
