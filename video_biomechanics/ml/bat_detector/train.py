"""Training script for bat endpoint detection model."""
...
try:
    from .model import create_model, BatKeypointLoss
    from .dataset import BatDataset, create_dataloaders
except ImportError:
    from model import create_model, BatKeypointLoss
    from dataset import BatDataset, create_dataloaders
...
def validate(
    ...
    Returns:
        loss: Average loss
        pixel_error: Average pixel error per endpoint
        length_error: Average bat length error (%)
    """
    model.eval()
    total_loss = 0.0
    ...
    with torch.no_grad():
        for images, targets in loader:
            ...
            pred = outputs.view(-1, 2, 2)  # (B, 2, 2)
            target = targets.view(-1, 2, 2)
            ...
            # Euclidean distance per endpoint
            dist = torch.sqrt(((pred_pixels - target_pixels) ** 2).sum(dim=-1))
            total_pixel_error += dist.mean().item() * images.size(0)

            # Bat length error
            pred_length = torch.sqrt(((pred_pixels[:, 1] - pred_pixels[:, 0]) ** 2).sum(dim=-1))
            target_length = torch.sqrt(((target_pixels[:, 1] - target_pixels[:, 0]) ** 2).sum(dim=-1))
            length_err = torch.abs(pred_length - target_length) / (target_length + 1e-6)
            total_length_error += length_err.mean().item() * images.size(0)

            count += images.size(0)

    n = len(loader.dataset)
    return total_loss / n, total_pixel_error / n, total_length_error / n
...
def train(
    ...
    """Train the bat endpoint detection model.
    ...
    # Loss and optimizer
    criterion = BatKeypointLoss(use_wing_loss=True)
    ...
    best_model_path = output_dir / "best_bat_model.pth"
    ...
    for epoch in range(epochs):
        ...
        # Validate
        val_loss, pixel_error, length_error = validate(
        ...
        print(f"Epoch {epoch+1:3d} | "
              ...
              f"Length Err: {length_error*100:.1f}%")
    ...
    with open(output_dir / "bat_training_config.json", 'w') as f:
...
def main():
    parser = argparse.ArgumentParser(description="Train bat endpoint detection model")
    parser.add_argument("--annotations", type=Path, default=Path("bat_annotations.json"),
    ...
    if not annotations_file.exists():
        ...
        print("Run the following to create annotations:")
        print("  1. python extract_frames.py  # Select frames from videos")
        print("  2. python annotate.py        # Annotate bat endpoints")
        return
    ...
    if not args.no_synthetic:
        synthetic_annotations_file = script_dir / "synthetic_bat_annotations.json"
